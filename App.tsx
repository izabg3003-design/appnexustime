...
const App: React.FC = () => {
  ...
  const loadUserData = useCallback(async (userId: string, retryCount = 0) => {
    try {
      ...
      if (profile) {
        // Sincronizar Nexus ID se estiver em falta no Supabase
        if (!profile.nexus_id) {
          const generatedId = userId.substring(0, 8);
          await supabase.from('profiles').update({ nexus_id: generatedId }).eq('id', userId);
          profile.nexus_id = generatedId;
        }
    ...
    <div className="min-h-screen text-slate-100 bg-[#020617] selection:bg-purple-500/30">
      ...
      {appState === 'landing' && (
        <LandingPage 
          ...
          onStartFree={() => { setLoginMode('register'); setAppState('login'); }}
          t={t} 
          lang={systemLang} 
      ...
      {appState === 'login' && <LoginPage onLogin={() => {}} onBack={() => setAppState('landing')} t={t} externalError={authError} initialRegister={loginMode === 'register'} />}
